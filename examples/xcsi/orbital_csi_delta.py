...
from utils import charge_name_parse, charges_draw_parse, calculate_xcsi, \
    calculate_origin_csi_without_napp, calculate_origin_csi
...
if __name__ == '__main__':
    ...
    CSI_NPY_STORE_PREFIX = r"D:\CODE\#DATASETS\FullDB\c2addonwork\xCSI_justangle"  # change this
    ...
    CSI_NPY_FILE_SUFFIX = "xCSI_justangle"
    _ = \
        """
    # =============== PART 2 ================ #
    2. Calculate if needed.
    ...
    for N in draw_list:
        ...
        enref=np.load((pathlib.Path(
            CSI_NPY_STORE_PREFIX) / f"C{N}_{CSI_NPY_FILE_SUFFIX}.npz").as_posix())["energy"]
        for charge in charged_list:
            charge_in_name = charge_name_parse(charge)
            ...
            xcsilist = calculate_origin_csi(csi_value, napp_value, charge)
            xcsilist_only_xcsi = calculate_origin_csi_without_napp(csi_value, napp_value, charge)
            enlist = np.array([item for item in energy_value])-enref
