...
from src.fullerenedataparser.molecular.fullerene import FullereneCage
...
NPY_DIR = r"D:\CODE\#DATASETS\FullDB"
OUTPUT_ROOT_DIR = r"E:\我的坚果云\draft\2022\xCSI\figures_draft"
...
if __name__ == '__main__':
    ...
    for N in draw_list:
        ...
        enref = np.load((pathlib.Path(
            rf"{NPY_DIR}\CSI") / f"C{N}_CSI.npz").as_posix())[
        ...
        for charge in charged_list:
            ...
            file = (pathlib.Path(
                rf"{NPY_DIR}\CSI" + charge_in_name) / f"C{N}_CSI.npz").as_posix()
            file = np.load(file)
            ...
            if True:
                if charge == 0:
                    ...
                    if charge > 0:
                        ...
                        ax.scatter(x, y, marker="x",
                                   color="tab:green")
                    ax.set_xlabel(r"$X^q_i+0.2\mathrm{NAPP}$", size=15)
                    ax.set_ylabel(
                ...
                # ax.set_axis_off()
                # ax.imshow(img,)
            with open(rf"{OUTPUT_ROOT_DIR}\CSINAPP\Pearson.txt", "a") as f:
                corr=np.corrcoef(np.array([napplist, csilist, enlist - min(enlist)]))
                print(f"{N} "
                      f"{charge} "
                      f"{corr[2,1] if charge else np.corrcoef(np.array([napplist,enref]))[0,1]}",
                      file=f)
        plt.savefig(
            rf"{OUTPUT_ROOT_DIR}\CSINAPP\C{N}_CSI.png",
            dpi=600)
        # if N == 60:
        #     ax = fig.add_subplot(spec[0, 1])
