...
if __name__ == '__main__':
    ...
    for N in draw_list:
        ...
        enref = np.load((pathlib.Path(
            rf"{NPY_DIR}\c2addonwork\xCSI_POAV") / f"C{N}_xCSI_POAV.npz").as_posix())[
        ...
        for charge in charged_list:
            ...
            file = (pathlib.Path(
                rf"{NPY_DIR}\c2addonwork\xCSI_POAV" + charge_in_name) / f"C{N}_xCSI_POAV.npz").as_posix()
            file = np.load(file)
            ...
            if True:
                ...
                # ax.set_axis_off()
                # ax.imshow(img,)
            with open(rf"{OUTPUT_ROOT_DIR}\xCSI_POAV\Pearson.txt","a") as f:
                ...
                print(f"{N} "
                      ...
                      file=f)
        plt.savefig(
            rf"{OUTPUT_ROOT_DIR}\xCSI_POAV\C{N}xCSI_POAV.png",
            dpi=600)
        # if N == 60:
        #     ax = fig.add_subplot(spec[0, 1])
