...
if __name__ == '__main__':
    ...
    for N in draw_list:
        ...
        enref = np.load((pathlib.Path(
            rf"{NPY_DIR}\xCSI_withdis") / f"C{N}_xCSI_withdis.npz").as_posix())[
        ...
        for charge in charged_list:
            ...
            file = (pathlib.Path(
                rf"{NPY_DIR}\xCSI_withdis" + charge_in_name) / f"C{N}_xCSI_withdis.npz").as_posix()
            file = np.load(file)
            ...
            if True:
                ...
                ax.set_axis_off()
                ax.imshow(img,)
            with open(rf"{OUTPUT_ROOT_DIR}\XCSIref\Pearson.txt","a") as f:
                ...
                print(f"{N} "
                      ...
                      file=f)
        plt.savefig(
            rf"{OUTPUT_ROOT_DIR}\CSINAPP\C{N}XCSIref.png",
            dpi=600)
        # if N == 60:
        #     ax = fig.add_subplot(spec[0, 1])
